...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript('/devoops/plugins/datatables/jquery.dataTables.js', function(){
			$.getScript('/devoops/plugins/datatables/ZeroClipboard.js', function(){
				$.getScript('/devoops/plugins/datatables/TableTools.js', function(){
					$.getScript('/devoops/plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript('/devoops/plugins/fineuploader/jquery.fineuploader-5.0.1.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript('/devoops/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript('/devoops/plugins/d3/d3.v3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript('/devoops/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript('/devoops/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript('/devoops/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript('/devoops/plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript('/devoops/plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
